...
const Relation = require('../../src/Lucid/Relations/Relation')
...
const MorphMany = require('../../src/Lucid/Relations/MorphMany')
const MorphOne = require('../../src/Lucid/Relations/MorphOne')
const MorphTo = require('../../src/Lucid/Relations/MorphTo')
...
describe('Relations', function () {
  ...
  context('MorphTo', function () {
    it('should return an instance of MorphTo when relation method has been called', function () {
      ...
      const tag = new Tag()
      expect(tag.taggable() instanceof MorphTo).to.equal(true)
    })
    ...
    it('should have proper foriegn and primary keys from the related model', function () {
      ...
      const tag = new Tag()
      expect(tag.taggable().toKey).to.equal('id')
      expect(tag.taggable().fromKey).to.equal('taggable_id')
      expect(tag.taggable().typeKey).to.equal('taggable_type')
    })

    it('should be able to fetch results from related model', function * () {
      const savedVideo = yield relationFixtures.createRecords(Database, 'videos', {title: 'Musical Routes (AdonisJs)', uri: 'https://youtu.be/w7LD7E53w3w'})
      const savedTag = yield relationFixtures.createRecords(Database, 'tags', {title: 'adonis', taggable_id: savedVideo[0], taggable_type: 'videos'})
      ...
      Relation.morphMap([Video])
      const tag = yield Tag.find(savedTag[0])
      const video = yield tag.taggable().fetch()
      expect(queryHelpers.formatQuery(videosQuery.sql)).to.equal(queryHelpers.formatQuery('select * from "videos" where "id" = ? limit ?'))
      expect(videosQuery.bindings).deep.equal(queryHelpers.formatBindings(savedVideo.concat([1])))
      expect(video instanceof Video).to.equal(true)
      expect(tag.taggable_id).to.equal(video.id)
      expect(tag.taggable_type).to.equal('videos')
      yield relationFixtures.truncate(Database, 'tags')
      yield relationFixtures.truncate(Database, 'videos')
    ...
    it('should be able to eagerLoad results from related model', function * () {
      ...
      Video.bootIfNotBooted()
      Relation.morphMap([Video])
      const tag = yield Tag.query().where('id', savedTag[0]).with('taggable').first()
      expect(queryHelpers.formatQuery(videosQuery.sql)).to.equal(queryHelpers.formatQuery('select * from "videos" where "id" in (?)'))
      expect(videosQuery.bindings).deep.equal(queryHelpers.formatBindings(savedVideo))
      expect(tag instanceof Tag).to.equal(true)
      expect(tag.get('taggable') instanceof Video).to.equal(true)
      expect(tag.get('taggable').id).to.equal(tag.taggable_id).to.equal(savedVideo[0])
      yield relationFixtures.truncate(Database, 'tags')
      yield relationFixtures.truncate(Database, 'videos')
    })
    ...
    it('should be able to eagerLoad results from related model instance', function * () {
      ...
      const tag = yield Tag.find(savedTag[0])
      expect(tag instanceof Tag).to.equal(true)
      yield tag.related('taggable').load()
      expect(queryHelpers.formatQuery(videosQuery.sql)).to.equal(queryHelpers.formatQuery('select * from "videos" where "id" = ? limit ?'))
      expect(videosQuery.bindings).deep.equal(queryHelpers.formatBindings(savedVideo.concat([1])))
      expect(tag.get('taggable') instanceof Video).to.equal(true)
      expect(tag.get('taggable').id).to.equal(tag.taggable_id).to.equal(savedVideo[0])
    ...
    it('should be able to define query constraints when eagerLoading via model instance', function * () {
      ...
      const tag = yield Tag.find(savedTag[0])
      expect(tag instanceof Tag).to.equal(true)
      yield tag.related('taggable').scope('taggable', function (builder) {
        builder.whereNull('created_at')
      }).load()
      expect(queryHelpers.formatQuery(videosQuery.sql)).to.equal(queryHelpers.formatQuery('select * from "videos" where "created_at" is null and "id" = ? limit ?'))
      expect(videosQuery.bindings).deep.equal(queryHelpers.formatBindings(savedVideo.concat([1])))
      expect(tag.get('taggable') instanceof Video).to.equal(true)
      expect(tag.get('taggable').id).to.equal(tag.taggable_id).to.equal(savedVideo[0])
      yield relationFixtures.truncate(Database, 'tags')
    ...
    it('should be able to eagerLoad multiple results from related model', function * () {
      ...
      Relation.morphMap([Video])
      let tags = yield Tag.query().with('taggable').fetch()
      tags = tags.toJSON()
      expect(tags[0].taggable_id).to.equal(tags[0].taggable.id)
      expect(tags[0].taggable_id).to.equal(tags[1].taggable_id)
      expect(tags[1].taggable_id).to.equal(tags[1].taggable.id)
      yield relationFixtures.truncate(Database, 'tags')
      yield relationFixtures.truncate(Database, 'videos')
    })
    ...
    it('should be able to associate a related model', function * () {
      ...
      const tag = new Tag()
      tag.title = 'adonis'
      tag.taggable().associate(video)
      yield tag.save()
      expect(tag.id).not.to.equal(undefined)
      expect(tag.taggable_id).to.equal(video.id)
      expect(tag.taggable_type).to.equal('videos')
      yield relationFixtures.truncate(Database, 'tags')
      yield relationFixtures.truncate(Database, 'videos')
    ...
    it('should throw an error when associate value is not an instance of related model', function * () {
      ...
      tag.title = 'adonis'
      const fn = function () {
        return tag.taggable().associate(video)
      }
      expect(fn).to.throw('E_INVALID_RELATION_INSTANCE: associate accepts an instance one of: Video, Place')
    })
    ...
    it('should throw an error when trying to associate a related model which is unsaved', function * () {
      ...
      expect(fn).to.throw('E_UNSAVED_MODEL_INSTANCE: Cannot perform associate on Video model since Tag instance is unsaved')
    })
    ...
    it('should throw an error when trying to call saveMany method on a morphTo relation', function * () {
      ...
      tag.title = 'adonis'
      try {
        yield tag.taggable().saveMany([video])
        expect(true).to.equal(false)
      } catch (e) {
        expect(e.name).to.equal('ModelRelationException')
    ...
    it('should throw an error when trying to call create method on a morphTo relation', function * () {
      ...
      tag.title = 'adonis'
      try {
        yield tag.taggable().create(video)
        expect(true).to.equal(false)
      } catch (e) {
        expect(e.name).to.equal('ModelRelationException')
    ...
    it('should throw an error when trying to call createMany method on a morphTo relation', function * () {
      ...
      tag.title = 'adonis'
      try {
        yield tag.taggable().createMany([video])
        expect(true).to.equal(false)
      } catch (e) {
        expect(e.name).to.equal('ModelRelationException')
    ...
    it('should be able to dissociate a related model', function * () {
      ...
      Relation.morphMap([Video])
      const tag = yield Tag.find(savedTag[0])
      tag.taggable().dissociate()
      yield tag.save()
      expect(tag.taggable_id).to.equal(null)
      expect(tag.taggable_type).to.equal(null)
      yield relationFixtures.truncate(Database, 'tags')
      yield relationFixtures.truncate(Database, 'videos')
    })
  })
  ...
  context('Regression:MorphOne', function () {
    ...
    it('should be able to delete related records', function * () {
      ...
      expect(queryHelpers.formatBindings(query.bindings)).deep.equal(['places', 1])
      yield relationFixtures.truncate(Database, 'places')
    })
  })
